...
// GET district GeoJSON by ISO code
app.get('/api/districts/:iso', (req, res) => {
  const iso = req.params.iso.toUpperCase().replace(/[^A-Z]/g, '');
  const allowed = ['SYR','LBN','IRQ','ISR','IRN','YEM','SAU','TUR','JOR','PSE'];
  if (!allowed.includes(iso)) {
    return res.status(400).json({ error: 'Unsupported country: ' + iso });
  }
  const filePath = path.join(__dirname, 'districts', iso + '.geojson');
  if (!fs.existsSync(filePath)) {
    return res.status(404).json({ error: 'GeoJSON not found for: ' + iso });
  }
  try {
    ...
    res.status(500).json({ error: 'Failed to read district file' });
  }
});

// GET district GeoJSON — paint state kaydet
app.post('/api/paint', (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: 'Failed to save paint state' });
  }
});
...
app.get('/api/paint', (req, res) => {
  ...
  try {
    res.json(JSON.parse(fs.readFileSync(paintPath, 'utf8')));
  } catch(e) {
    res.json([]);
  }
});
