...
// GDELT proxy — CORS sorununu aşmak için sunucu üzerinden çek
app.get('/api/gdelt', async (req, res) => {
  const timespan = req.query.timespan || '24h';
  const q = encodeURIComponent('(missile attack airstrike explosion military Iran IRGC Houthi Hezbollah Hamas nuclear strike intercept)');
  const url = `https://api.gdeltproject.org/api/v2/geo/geo?query=${q}&timespan=${timespan}&format=GeoJSON&maxrecords=250&SMOOTHING=0`;
  try {
    ...
    res.status(500).json({ error: 'GDELT fetch failed', detail: err.message });
  }
});
